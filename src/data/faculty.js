export const FACULTY = [
  {
    name: 'Kenneth Leonard',
    headshot: '/Images/Headshots/DSC09920.JPG',
    title: 'Lead Pastor · Music Producer · Grammy-Credited Songwriter',
    summary: 'Co-Lead Pastor of The Purpose Place. Grammy-nominated songwriter and producer whose leadership bridges ministry and music with excellence.',
    bio: `Kenneth Leonard Jr. is a pastor, creative leader, and Grammy-nominated songwriter whose influence bridges ministry and music with excellence and integrity. With decades of experience serving in church leadership and creative direction, Kenneth has cultivated environments where worship, discipleship, and purpose intersect.

His musical contributions include songwriting and production work connected to Grammy-nominated projects and Dove Award-recognized achievements, reflecting his impact within the gospel music industry. His creative leadership has extended to mentoring musicians and developing artists through initiatives focused on excellence, spiritual depth, and innovation.

Kenneth faithfully served for over a decade in high-level ministry leadership roles before stepping fully into his pastoral calling. Today, he serves as Co-Lead Pastor of The Purpose Place alongside his wife, Tasha. Together, they lead a growing ministry centered on spiritual maturity, identity, and purposeful living. Kenneth's leadership style combines strategic vision with pastoral care, empowering individuals and families to grow in faith while building strong, healthy communities.

In addition to his pastoral and creative work, Kenneth remains committed to theological development and leadership growth, continually investing in education and mentorship. His passion is to see believers equipped not only spiritually but also practically, walking confidently in their God-given assignment.`,
  },
  {
    name: 'Tasha Cobbs Leonard',
    headshot: '/Images/Headshots/DSC09956.JPG',
    title: 'Multi-Award-Winning Gospel Artist · Pastor · Cultural Influencer',
    summary: '3x Grammy, 12x Dove, and 16x Stellar Award winner. Co-Lead Pastor of The Purpose Place. Her anthem "Break Every Chain" defined a generation of worship.',
    bio: `Tasha Cobbs Leonard is a multi-award-winning gospel artist, pastor, and cultural leader whose voice and influence have shaped contemporary worship around the world. Recognized as one of the most impactful figures in modern gospel music, she has earned 3 Grammy Awards, 12 Dove Awards, and 16 Stellar Awards throughout her career.

Her breakout album Grace, featuring the globally recognized anthem "Break Every Chain," propelled her to the forefront of gospel music and established her as a defining voice for her generation. Multiple projects have debuted at No. 1 on the Billboard Gospel charts, and her music has resonated with millions across churches and platforms worldwide.

Beyond her achievements in music, Tasha's ministry impact extends to national and global stages. She has appeared on major television platforms including Good Morning America, BET's Black Girls Rock, and CNN's Fourth of July celebration, using her voice to amplify messages of faith, hope, and empowerment.

In addition to her music career, Tasha serves as Co-Lead Pastor of The Purpose Place alongside her husband, Kenneth Leonard Jr. Through pastoral leadership, mentorship, and teaching, she is deeply committed to helping individuals embrace healing, identity, and purpose. Her ministry is marked not only by powerful worship moments but also by strategic leadership and a passion for building strong, purpose-driven communities.`,
  },
  {
    name: 'Todd Galberth',
    headshot: '/Images/Headshots/DSC09933.JPG',
    title: 'Billboard #1 Artist · Award-Winning Songwriter · Executive Pastor',
    summary: 'Billboard #1 gospel artist and BMI Gospel Song of the Year recipient. Executive Pastor at Change Church. Founder of Worship Changed My Life.',
    bio: `Todd Galberth is a dynamic gospel recording artist, songwriter, worship leader, and executive pastor whose voice and leadership continue to shape the modern worship culture across churches and audiences worldwide. Known for his soulful authenticity and spiritual depth, Todd leads worship experiences that invite people into transformative encounters with God.

His sophomore project, Decrease, debuted at #1 on the Billboard Gospel charts, solidifying his place among today's leading voices in worship music. He also received BMI's Gospel Song of the Year Award for the powerful anthem "I'm Getting Ready." In 2025, Todd released Greater, a live worship project that became both a sound and a movement — calling believers to elevate their expectations and trust God for more.

Beyond music, Todd is the founder of Worship Changed My Life, a lifestyle and apparel brand designed to spark conversations about the transformative power of worship and faith in everyday life.

Todd currently serves as Executive Pastor at Change Church under the leadership of Dr. Dharius Daniels. Change Church is a thriving multi-campus ministry with locations in Georgia and New Jersey, along with a growing global online congregation. Each week, thousands gather across these campuses to experience worship, teaching, and community.

Originally from Raeford, North Carolina, Todd now resides in Georgia with his wife, Vanessa, and their two children. He often reminds those he leads: "God is looking for a worshipper, not a song."`,
  },
  {
    name: 'Naomi Raine',
    headshot: '/Images/Headshots/DSC09947.JPG',
    title: '6x Grammy Winner · Executive Worship Pastor · Founder of ReFraine Records',
    summary: '6x Grammy Award winner and 10x Grammy-nominated artist. Executive Worship Pastor at Fresh Start Christian Center. Founder of ReFraine Records.',
    bio: `Naomi Raine began singing at two years old and wrote her first song by the age of seven. What started as an early passion has developed into a multifaceted career spanning global ministry, executive leadership, and creative direction.

She is a 6x Grammy Award-winning and 10x Grammy-nominated artist and has also received NAACP Image, Stellar, BET, Billboard, and Dove Awards. She has contributed to globally recognized worship songs including "Jireh," "Promises," and "The Story I'll Tell."

Since 2018, Naomi has served as Worship Pastor at Fresh Start Christian Center. With over eight years of leadership experience, she oversees worship, creative direction, and production strategy across the ministry. She plays a key role in developing the structure and flow of every service expression, from worship nights to large-scale gatherings.

Naomi is also a member of the church's Executive Leadership Team, where she contributes both spiritually and strategically to the broader vision of the ministry. Her responsibilities include training and developing worship teams, overseeing production systems, and directing large-scale creative presentations including Broadway-style theatrical experiences.

In addition to her pastoral leadership, Naomi is the founder of ReFraine Records, a label dedicated to developing and positioning emerging artists for long-term impact. Whether leading worship, coaching creatives, or overseeing production, Naomi is committed to excellence, structure, and building healthy creative environments.`,
  },
  {
    name: 'Jane Williams',
    headshot: '/Images/Headshots/Jane-Williams.png',
    headshotPosition: 'center 15%',
    title: 'Songwriter · Worship Leader · Creative Strategist · Worship Culture Architect',
    summary: '15+ years equipping churches to build thriving worship cultures. Former Elevation Worship leader and contributing songwriter behind "Same God," "Do It Again," and "Resurrecting."',
    bio: `Jane Williams is a songwriter, worship leader, and creative strategist with over 15 years of experience equipping churches to cultivate thriving worship and creative cultures. Widely respected for her leadership within large-scale ministry environments, Jane spent 11 years with Elevation Worship, serving as both a worship leader and contributing songwriter during one of the most influential seasons in the collective's global growth.

Elevation Worship, a Grammy Award–winning collective with billions of global streams, multiple RIAA-certified Gold and Platinum records, and numerous Billboard No. 1 Christian chart placements, has shaped modern worship worldwide. During her tenure, Jane helped build campus bands, pastor worship teams, develop sustainable songwriting systems, and mentor emerging worship leaders across multiple campuses of Elevation Church.

Jane contributed to songs such as "Same God," "Do It Again," "Resurrecting," and other widely sung anthems that have been embraced by churches around the world. Her leadership extended beyond the platform, serving as a creative producer, online campus host, and media personality — helping shape the church's digital presence and global engagement.

Since transitioning to freelance ministry work in 2022, Jane has partnered with churches nationwide to build songwriting culture, strengthen worship leader development, and create healthy, sustainable creative teams. Her work focuses on merging spiritual depth with strategic systems that support long-term growth. In addition, she collaborates with faith-based brands to create devotionals, voiceover narration, and brand-aligned content rooted in thoughtful theology and storytelling.

Jane holds a Bachelor of Arts with a double major in History and Communication Studies from Westmont College in Santa Barbara, California. She resides in Charlotte, North Carolina with her husband, Luke, and their 3 children.`,
  },
  {
    name: 'Mack Brock',
    headshot: '/Images/Headshots/Mack-Brock.png',
    headshotPosition: 'center 10%',
    title: 'Songwriter · Worship Leader · Billboard No. 1 Writer · Worship Artist',
    summary: '20+ years shaping modern worship culture. Co-wrote "O Come to the Altar," "Do It Again," and "Resurrecting." Former Elevation Worship producer and worship leader.',
    bio: `Mack Brock is a worship leader, songwriter, and recording artist with over 20 years of experience shaping modern worship culture. His songwriting has helped define a generation of congregational music, with songs that are sung weekly in churches across the globe.

Mack co-wrote widely recognized worship anthems including "O Come to the Altar," "Do It Again," "Resurrecting," "Greater Things," and "I Am Loved." These songs have collectively generated hundreds of millions of global streams, reached No. 1 on Billboard Christian charts, and been part of projects that earned RIAA Gold and Platinum certifications. "O Come to the Altar" alone became one of the most sung worship songs worldwide and remains a staple in church repertoires internationally.

He spent 10 years with Elevation Worship as a music producer and worship leader during a defining season of growth for the Grammy Award–winning collective. During his tenure, Elevation Worship became one of the most influential worship movements in the world, with multiple chart-topping albums and global impact. Mack contributed not only as a songwriter and worship leader but also as a producer, helping shape the sonic identity of songs that would reach millions.

Growing up as a pastor's kid, Mack began leading worship in his father's church while still in high school. That early foundation continues to shape his ministry today. Despite decades of influence and success, his deepest passion remains unchanged: to see people encounter Jesus through worship and experience the transformative power of God's presence in a gathered community.

Mack resides in Charlotte, North Carolina with his wife and their 3 children, where he continues to write, lead, and serve the local and global Church.`,
  },
]
